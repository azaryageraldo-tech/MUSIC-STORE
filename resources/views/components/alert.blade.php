...
@php
    $classes = match($type) {
        ...
        default => 'bg-gray-50 text-gray-800 border-gray-500'
    };
...
<div {{ $attributes->merge(['class' => "flex p-4 mb-4 border-l-4 rounded-r-lg {$classes}"]) }} role="alert">
    @if($icon)
        <div class="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 mr-2">
            {!! $iconSvg !!}
        </div>
    @endif
    ...
    @if($dismissible)
        <button type="button" class="ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 focus:ring-gray-400 p-1.5 inline-flex h-8 w-8 {{ str_replace('bg-', 'hover:bg-', $classes) }}" aria-label="Close" onclick="this.parentElement.remove()">
